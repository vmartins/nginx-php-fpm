...
if [ ! -z "$NGINX_FASTCGI_READ_TIMEOUT" ]; then
    sed -i "s/fastcgi_read_timeout 60;/fastcgi_read_timeout ${NGINX_FASTCGI_READ_TIMEOUT};/g" /etc/nginx/sites-available/default.conf
    if [ -f /etc/nginx/sites-available/default-ssl.conf ]; then
        sed -i "s/fastcgi_read_timeout 60;/fastcgi_read_timeout ${NGINX_FASTCGI_READ_TIMEOUT};/g" /etc/nginx/sites-available/default-ssl.conf
    fi
fi
